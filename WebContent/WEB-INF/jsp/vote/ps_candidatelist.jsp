...
<br>
<form id="searchForm" action="${root}/vote/ps_candidate.do">
...
<s:if test="vote.status == \"1.new\"&&vote.primaryVote==null">
	<a class="upload" href="${root}/vote/ps_candidate!importForm.do?vote.id=<s:property value="vote.id"/>" >Excel导入</a> 
	<a class="ui_icon_circle_plus" href="${root}/vote/ps_candidateinput!input.do?vote.id=<s:property value="vote.id"/>" >新增候选人</a>  
</s:if>
...
<s:if test="vote.status == \"1.new\"&&vote.primaryVote!=null">
<form id="newCandidateForm" action="${root}/vote/ps_candidate!createSecondaryCandidates.do">
<s:hidden name="vote.id"/>
...
<table width="100%" class="table" align="center" cellpadding="1" cellspacing="0" bgcolor="#FFFFFF">
	...
	<s:iterator value="objectList" status="status">
		<tr>
			...
			<s:if  test="vote.primaryVote==null">
			<td align="left" class='td_body'><a href="${root}/vote/ps_candidate!show2ndaryCandidates.do?vote.id=<s:property value="vote.id"/>&candidate.id=<s:property value="id"/>"><s:property value="contain2ndCandidateTXT"/></a></td>
			</s:if>
			...
			<s:if test="vote.status == \"1.new\"">
				<a class="user-edit" href="${root}/vote/ps_candidateinput!input.do?vote.id=<s:property value="vote.id"/>&candidate.id=<s:property value="id"/>">编辑</a>&nbsp;&nbsp;				
				<a class="user-delete" href="${root}/vote/ps_candidate!delete.do?vote.id=<s:property value="vote.id"/>&candidate.id=<s:property value="id"/>" onclick="return fDelCheck()">删除</a>&nbsp;&nbsp;
			</s:if>	
