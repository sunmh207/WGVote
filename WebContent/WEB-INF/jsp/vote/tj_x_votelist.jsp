...
<div style="float: left; padding-bottom: 0px;">
	  ...
	  <s:if test="primaryVote.status == \"4.done\"">
	  	<a class="ui_icon_circle_plus" href="${root}/vote/tj_vote!copy2secondaryVote.do?primaryVote.id=<s:property value="primaryVote.id"/>"  onclick="return fCopy2secondaryVoteCheck()">发起二次投票</a> 
	  </s:if>
	  <a class="return" href="${root}/vote/tj_vote.do" >返回投票列表</a> 
</div>
	 ...
	 <s:iterator value="objectList" status="st">
	 	<s:if test="status == \"1.new\"">
			<tr bgcolor="#FFFF00">
				...
				<td align="center">
					...
					<a class="statics" href="${root}/vote/tj_vote!ready.do?primaryVote.id=<s:property value="primaryVote.id"/>&vote.id=<s:property value="id"/>"  onclick="return fReadyCheck()">准备投票</a>&nbsp;&nbsp;
					<a class="user-delete" href="${root}/vote/tj_vote!delete.do?primaryVote.id=<s:property value="primaryVote.id"/>&vote.id=<s:property value="id"/>" onclick="return fDelCheck()">删除</a>&nbsp;&nbsp;
				</td>
			</tr>
		</s:if>
		...
		<tr bgcolor="#00FF00">
			...
			<td align="center">
				...
				<a class="statics" href="${root}/vote/tj_vote!start.do?primaryVote.id=<s:property value="primaryVote.id"/>&vote.id=<s:property value="id"/>"  onclick="return fStartCheck()">开始</a>&nbsp;&nbsp;
			</td>
		</tr>
		</s:if>
